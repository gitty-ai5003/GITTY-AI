        ...
        for mod in modules:
            ...
            for cls in mod.classes:
                ...
                for base in cls.bases:
                    ...
                    # Target node is hash of the base name (if local)
                    base_node_id = hashlib.sha256(f"{file_node.id}:{base}".encode()).hexdigest()
                    rel = self.rb.build_relationship(class_node.id, base_node_id, "INHERITS", {"base_name": resolved_base})
                    self.repository.add_edge(rel.source_node, rel.target_node, rel.relationship_type, rel.metadata)
                    edges_count += 1
